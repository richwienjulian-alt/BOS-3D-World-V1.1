    ...
    function update(delta) {
      ...
      if (state === "CALL_RECEIVED") {
        ...
        if (completionSettlementStarted) {
          ...
          var mission002Baseline = mission002.getSharedStartBaselineStatus ? mission002.getSharedStartBaselineStatus() : null;
          var mission002BaselineReady = !!mission002Baseline && mission002Baseline.ready === true;
          completionSettlementBlockers = [];
          if (!sharedNetworkReady) completionSettlementBlockers.push("SHARED_NETWORK_NOT_READY");
          if (!cellLoadsReady) completionSettlementBlockers.push("CELL_LOADS_NOT_BASELINE");
          ...
          if (!mission002BaselineReady) completionSettlementBlockers.push("MISSION_002_BASELINE_NOT_READY");
          if (sharedNetworkReady && cellLoadsReady && priorityReady && responseHandoffReady && mission002BaselineReady) {
            if (!response.finalizeForSharedHandoff || response.finalizeForSharedHandoff() !== true) return;
            ...
            completionSettlementElapsed = 0;
            completionSettlementBlockers = [];
          } else if (completionSettlementElapsed + 1e-9 >= finite(sequence.completionSettlementMaximumSeconds, 8.0)) {
            fail("Mission 004 completion settlement exceeded the 8 second safety deadline. Pending: " +
