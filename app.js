...
const cameraControlForward = document.getElementById("camera-control-forward");
const cameraControlBackward = document.getElementById("camera-control-backward");
const cameraControlLeft = document.getElementById("camera-control-left");
const cameraControlRight = document.getElementById("camera-control-right");
const cameraControlRotateLeft = document.getElementById("camera-control-rotate-left");
const cameraControlRotateRight = document.getElementById("camera-control-rotate-right");
const cameraControlZoomOut = document.getElementById("camera-control-zoom-out");
const cameraControlZoomIn = document.getElementById("camera-control-zoom-in");
const cameraControlHome = document.getElementById("camera-control-home");
...
function getCustomerIncidentPresentation(missionId, runtimeState) {
  const plan = window.MISSION_BOS_CUSTOMER_INCIDENT_PRESENTATION_PLAN;
  if (!plan || !plan.missions || !missionId) return null;
  const mission = plan.missions[missionId];
  if (!mission || !mission.states) return null;
  const statePresentation = runtimeState && mission.states[runtimeState] ? mission.states[runtimeState] : null;
  if (!statePresentation) return null;
  return {
    title: mission.title || null,
    summaryPhase: statePresentation.summaryPhase || null,
    statusBadge: statePresentation.statusBadge || null,
    stage: statePresentation.stage || null,
    description: statePresentation.description || null
  };
}
  ...
  updateMissionPanel() {
    if (RECOVERY_CONFIG.enableValidatedMission001) {
      ...
      if (runtime) {
        ...
        const customerPresentation = getCustomerIncidentPresentation(missionId, runtimeState);

        missionPhaseValue.textContent = customerPresentation && customerPresentation.summaryPhase
          ? customerPresentation.summaryPhase
          : runtime.getPhaseLabel();
        missionStageValue.textContent = customerPresentation && customerPresentation.stage
          ? customerPresentation.stage
          : runtime.getStageLabel();
        missionTitleValue.textContent = customerPresentation && customerPresentation.title
          ? customerPresentation.title
          : (selectedPlan
            ? (selectedPlan.shortTitle || (selectedPlan.incidentReference && selectedPlan.incidentReference.title) || selectedPlan.title)
            : "Missionen");
        missionStatusValue.textContent = customerPresentation && customerPresentation.statusBadge
          ? customerPresentation.statusBadge
          : (customerStatusByState && runtimeState && customerStatusByState[runtimeState]
            ? customerStatusByState[runtimeState]
            : runtime.getStatusLabel());
        missionDescriptionValue.textContent = customerPresentation && customerPresentation.description
          ? customerPresentation.description
...
const presenterCameraAdapter = {
  ...
  releasePresenterCamera(reason) {
    if (validatedPresenter && typeof validatedPresenter.notifyManualInput === "function") {
      // Reuse the established manual-camera release path; this never changes guided/demo mode itself.
      validatedPresenter.notifyManualInput("MOUSE_WHEEL");
    }
    return true;
  },
  ...
  rotateYaw(deltaRadians, reason) {
    ...
    this.releasePresenterCamera(reason || "DASHBOARD_CAMERA");
    cameraVelocity.set(0, 0, 0);
    targetYaw += delta;
    return targetYaw;
  },

  panGround(forwardMeters, rightMeters, reason) {
    this.releasePresenterCamera(reason || "TOUCH_CAMERA");
    cameraVelocity.set(0, 0, 0);
    ...
    camera.position.addScaledVector(forward, Number(forwardMeters) || 0);
    camera.position.addScaledVector(right, Number(rightMeters) || 0);
    ...
    camera.position.y = freeCameraHeight;
    return true;
  },

  setFov(nextFov, reason) {
    this.releasePresenterCamera(reason || "TOUCH_CAMERA");
    const fov = THREE.MathUtils.clamp(Number(nextFov) || camera.fov, 36, 78);
    if (Math.abs(camera.fov - fov) > 0.0001) {
      camera.fov = fov;
      camera.updateProjectionMatrix();
    }
    return camera.fov;
  ...
  goHome(reason) {
    this.releasePresenterCamera(reason || "DASHBOARD_CAMERA");
    const contract = window.MISSION_BOS_INITIAL_CAMERA_CONTRACT_013M8;
    const pose = contract && contract.initialPose ? contract.initialPose : {
      position: { x: 0.78, y: 9, z: 46 },
      yaw: 0,
      pitch: -0.17863100651394934,
      fov: 56
    };
    const applied = this.applyPose(pose);
    if (applied) freeCameraHeight = Number(pose.position.y);
    return applied;
  }
};
...
if (window.MissionBosTouchCameraController && window.MISSION_BOS_TOUCH_CAMERA_PLAN &&
    window.MISSION_BOS_TOUCH_CAMERA_PLAN_VALIDATION &&
    window.MISSION_BOS_TOUCH_CAMERA_PLAN_VALIDATION.status === "PASSED") {
  const touchRuntime = window.MissionBosTouchCameraController.create({
    plan: window.MISSION_BOS_TOUCH_CAMERA_PLAN,
    validation: window.MISSION_BOS_TOUCH_CAMERA_PLAN_VALIDATION,
    canvas: renderer.domElement,
    cameraAdapter: presenterCameraAdapter,
    inspectionRuntime: validatedNetworkInspection,
    controls: {
      forward: cameraControlForward,
      backward: cameraControlBackward,
      left: cameraControlLeft,
      right: cameraControlRight,
      rotateLeft: cameraControlRotateLeft,
      rotateRight: cameraControlRotateRight,
      zoomOut: cameraControlZoomOut,
      zoomIn: cameraControlZoomIn,
      home: cameraControlHome
    }
  });
  const touchManifest = touchRuntime.getManifest();
  const touchSafety = touchRuntime.getSafetyStatus();
  if (touchManifest && touchManifest.status === "PASSED" && touchSafety && touchSafety.status === "PASSED") {
    validatedTouchCamera = touchRuntime;
  } else {
    console.error("Touch/tablet camera runtime validation failed. Desktop controls remain active.");
  }
}

uiManager.updateAll(true);
window.addEventListener("beforeunload", function () {
  if (validatedTouchCamera && typeof validatedTouchCamera.dispose === "function") validatedTouchCamera.dispose();
